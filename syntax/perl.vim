...
syn match perlMethodSignature +\_s*\%((\_[^)]*)\)\?\_s*\|+                 contained contains=@perlExpr,perlStatementProc  nextgroup=perlSubAttributes
syn match perlFunction        +\<\%(class\|role\|extends\|with\)\>\_s*+                                                    nextgroup=perlPackageRef
syn match perlFunction        +\<\%(method\|before\|after\|around\|override\|augment\|test\)\>\_s*+                              nextgroup=perlMethodName
...
syn match perlStatementFlow		"\<\%(caller\|die\|dump\|eval\|try\|catch\|finally\|exit\|wantarray\)\>"
...
syn match  perlVarPlain          "__PACKAGE__" nextgroup=perlMethod
...
source ~/.vim/syntax/perlapos.vim
source ~/.vim/syntax/perlexport.vim
